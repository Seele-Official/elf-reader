...
struct test_constructior_args_count{
    static_assert(std::is_same_v<T, std::remove_reference_t<T>>, "T must be a non-reference type");
    static constexpr bool value = test_constructior_args_count<T, N-1, N-1, Is...>::value;
};

template <typename T, size_t... Is>
struct test_constructior_args_count<T, 0, Is...>{
    static_assert(std::is_same_v<T, std::remove_reference_t<T>>, "T must be a non-reference type");
    static constexpr bool value = requires { T{any_cast{Is}...}; };
};

template <typename T, size_t N, size_t... Is>
static constexpr bool test_constructior_args_count_v = test_constructior_args_count<T, N, Is...>::value;


template <typename T, size_t N = 0>
constexpr size_t member_count(){
    static_assert(std::is_same_v<T, std::remove_reference_t<T>>, "T must be a non-reference type");
    if constexpr(test_constructior_args_count_v<T, N> && !test_constructior_args_count_v<T, N+1>){
        return N;
    } else {
...
struct reflect {
    static_assert(std::is_same_v<T, std::remove_reference_t<T>>, "T must be a non-reference type");
    constexpr inline static size_t m_count = member_count<T>();
...
void forech_member(object_t&& object, callback_t&& callback) {
    iterate_tuple(reflect<std::remove_reference_t<object_t>>::collect_members(object), std::forward<callback_t>(callback));
}
